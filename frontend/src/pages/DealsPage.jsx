...
export default function DealsPage() {
  ...
  const loadDeal = async (id) => {
    try { const { data } = await api.get(`/deals/${id}`); setSelected(data) } catch(e) { toast.error('Ошибка загрузки') }
  }
  ...
  const sendMessage = async () => {
    ...
    setWorking(true)
    try {
      await api.post(`/deals/${selected._id}/message`, { text: msgText })
      setMsgText('')
      loadDeal(selected._id)
  ...
  const deliver = async () => {
    ...
    setWorking(true)
    try {
      await api.post(`/deals/${selected._id}/deliver`, { deliveryData: delivery })
  ...
  const confirm = async () => {
    ...
    setWorking(true)
    try {
      await api.post(`/deals/${selected._id}/confirm`)
  ...
  const dispute = async () => {
    ...
    setWorking(true)
    try {
      await api.post(`/deals/${selected._id}/dispute`, { reason })
  ...
  const isBuyer  = d => d?.buyer && (d.buyer._id||d.buyer)===user?._id
  const isSeller = d => d?.seller && (d.seller._id||d.seller)===user?._id
        ...
        <div>
          <div style={{ display:'flex', gap:6, marginBottom:16 }}>
            {[['all','Все'],['buyer','Покупки'],['seller','Продажи']].map(([v,l])=>(
              <button key={v} onClick={()=>setRole(v)} style={{
                flex:1, padding:'8px', borderRadius:8, border:'1px solid', cursor:'pointer', fontSize:12, fontWeight:700, fontFamily:'var(--font-h)', transition:'all 0.15s',
          ...
          {loading ? (
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {Array(4).fill(0).map((_,i)=><div key={i} className="skel" style={{height:80}}/>)}
            </div>
          ) : deals.length===0 ? (
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {deals.map(d=>(
                <div key={d._id} onClick={()=>loadDeal(d._id)} style={{
                  background: selected?._id===d._id ? 'var(--bg3)' : 'var(--bg2)',
                  border:`1px solid ${selected?._id===d._id ? 'rgba(245,200,66,0.3)' : 'var(--border)'}`,
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'16px 20px', display:'flex', flexDirection:'column', gap:10, minHeight:200, maxHeight:300 }}>
                {selected.messages?.map((m,i)=>(
                  <div key={i} style={{
                    ...
                    background: m.isSystem ? 'rgba(245,200,66,0.08)' : String(m.sender)===user._id ? 'rgba(124,106,255,0.15)' : 'var(--bg3)',
                    alignSelf: m.isSystem ? 'center' : String(m.sender)===user._id ? 'flex-end' : 'flex-start',
                ...
                <div style={{ padding:'12px 20px', borderTop:'1px solid var(--border)' }}>
                  {isSeller(selected) && !selected.deliveredAt && (
                    <div style={{ marginBottom:10 }}>
                      <textarea className="inp" rows={2} placeholder="Введите данные товара (логин/пароль, ключ и т.д.)" value={delivery} onChange={e=>setDelivery(e.target.value)} style={{ resize:'none', marginBottom:8 }}/>
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    <input className="inp" placeholder="Сообщение..." value={msgText} onChange={e=>setMsgText(e.target.value)} onKeyDown={e=>e.key==='Enter'&&sendMessage()} style={{ flex:1 }}/>
