...
export default function ProductPage() {
  ...
  const buy = async () => {
    ...
    setBuying(true)
    try {
      const { data } = await api.post('/deals', { productId: product._id })
  ...
  const isMine = user && String(seller?._id) === user._id
    ...
    <div style={{ maxWidth:940, margin:'0 auto', padding:'32px 20px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:28 }}>
        ...
        <div>
          ...
          <h1 style={{ fontFamily:'var(--font-h)', fontWeight:800, fontSize:26, lineHeight:1.2, marginBottom:12 }}>
            {product.title}
          </h1>
          ...
          {isMine && (
            <div className="badge badge-yellow" style={{ marginBottom:16 }}>Ваш товар</div>
          )}
          {product.status!=='active' && !isMine && (
            <div className="badge badge-red" style={{ marginBottom:16 }}>Товар недоступен</div>
          )}

          {/* Stats */}
          <div style={{ display:'flex', gap:16, color:'var(--t4)', fontSize:12 }}>
            <span>👁 {product.views||0} просмотров</span>
            <span>♡ {product.favorites?.length||0} в избранном</span>
          </div>
        </div>
      </div>
