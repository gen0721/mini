...
export default function HomePage() {
  ...
  useEffect(() => {
    api.get('/products?limit=8&sort=newest').then(r => setProducts(r.data.products||[])).catch(()=>{}).finally(()=>setLoading(false))
  }, [])
    ...
    <div>
      ...
      <section style={{
        ...
        <div style={{ position:'relative', maxWidth:760, margin:'0 auto' }}>
          <div className="badge badge-yellow" style={{ marginBottom:20, display:'inline-flex' }}>
          ...
          <h1 style={{
            ...
            <span style={{ color:'var(--accent)' }}>безопасно</span>
          </h1>
          <p style={{ color:'var(--t2)', fontSize:18, lineHeight:1.6, marginBottom:36, maxWidth:520, margin:'0 auto 36px' }}>
            Тысячи цифровых товаров с защитой сделки. Деньги переводятся продавцу только после вашего подтверждения.
          </p>
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
            <Link to="/catalog" className="btn btn-primary" style={{ padding:'14px 32px', fontSize:16 }}>
              Смотреть каталог →
            </Link>
            {!user && (
              <Link to="/auth?mode=register" className="btn btn-secondary" style={{ padding:'14px 32px', fontSize:16 }}>
                Зарегистрироваться
              </Link>
            )}
          </div>
          ...
          {CATEGORIES.map(cat => (
            <Link key={cat.slug} to={`/catalog?category=${cat.slug}`} style={{ textDecoration:'none' }}>
              ...
              onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.transform='none'}}>
                <div style={{ fontSize:28, width:44, height:44, borderRadius:12, background:'var(--bg3)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  {cat.icon}
                </div>
                <div>
      ...
      <section style={{ padding:'0 20px 60px', maxWidth:1200, margin:'0 auto' }}>
        ...
        {loading ? (
          <div className="grid-4">
            {Array(8).fill(0).map((_,i)=><div key={i} className="skel" style={{height:280}}/>)}
          </div>
        ) : products.length === 0 ? (
          ...
          <div className="grid-4">
            {products.map((p,i) => <ProductCard key={p._id} product={p} style={{ animationDelay:`${i*50}ms` }} />)}
          </div>
        )}
      </section>
      ...
      <section style={{ padding:'60px 20px', background:'var(--bg2)', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          <h2 style={{ fontFamily:'var(--font-h)', fontWeight:800, fontSize:28, textAlign:'center', marginBottom:40 }}>
            Почему выбирают нас
          </h2>
          <div className="grid-4">
            {FEATURES.map(f => (
      ...
      {!user && (
        <section style={{ padding:'80px 20px', textAlign:'center', maxWidth:600, margin:'0 auto' }}>
          <h2 style={{ fontFamily:'var(--font-h)', fontWeight:800, fontSize:36, marginBottom:16 }}>
            Готов начать?
          </h2>
          <p style={{ color:'var(--t2)', marginBottom:28, lineHeight:1.7 }}>
            Зарегистрируйся за 30 секунд через Telegram бот и начни торговать прямо сейчас.
          </p>
          <Link to="/auth?mode=register" className="btn btn-primary" style={{ padding:'16px 40px', fontSize:16 }}>
            Создать аккаунт →
          </Link>
        </section>
      )}
