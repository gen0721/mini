...
export default function WalletPage() {
  ...
  useEffect(() => {
    ...
    api.get('/wallet/transactions').then(r=>setTxs(r.data.transactions||[])).catch(()=>{}).finally(()=>setLoading(false))
  }, [user])
    ...
    <div style={{ maxWidth:700, margin:'0 auto', padding:'32px 20px' }}>
      ...
      <div style={{
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginTop:20 }}>
          <button className="btn btn-primary" onClick={()=>setModal('deposit')}>↓ Пополнить</button>
          <button className="btn btn-secondary" onClick={()=>toast.error('Вывод через CryptoBot — обратитесь в поддержку')}>↑ Вывести</button>
        </div>
      </div>

      {/* Stats */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:28 }}>
        {[['Пополнено','$'+parseFloat(user.totalDeposited||0).toFixed(2)],['Выведено','$'+parseFloat(user.totalWithdrawn||0).toFixed(2)],['Сделок',user.totalPurchases+user.totalSales||0]].map(([l,v])=>(
              ...
              <div key={tx._id||i} style={{
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ fontSize:11, color:'var(--t3)', marginTop:2, display:'flex', gap:8 }}>
                    <span>{new Date(tx.createdAt).toLocaleString('ru',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}</span>
      ...
      {/* Deposit Modal */}
      {modal==='deposit' && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', backdropFilter:'blur(12px)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }} onClick={()=>setModal(null)}>
          <div onClick={e=>e.stopPropagation()} style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:24, padding:28, width:'100%', maxWidth:420, animation:'fadeUp 0.25s ease' }}>
            ...
            <div style={{ display:'flex', gap:6, marginBottom:12 }}>
              {[5,10,25,50].map(v=>(
                <button key={v} onClick={()=>setAmount(String(v))} style={{
                  flex:1, padding:'8px', borderRadius:8, border:'1px solid', cursor:'pointer', fontSize:13, fontWeight:700, fontFamily:'var(--font-h)', transition:'all 0.15s',
            ...
            <input className="inp" type="number" placeholder="Сумма в USD" value={amount} onChange={e=>setAmount(e.target.value)} style={{ marginBottom:16, fontSize:20, fontFamily:'var(--font-h)', fontWeight:700, textAlign:'center' }}/>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 2fr', gap:10 }}>
              ...
              <button className="btn btn-primary" onClick={deposit} disabled={working}>
                {working ? <span style={{width:16,height:16,border:'2px solid transparent',borderTopColor:'currentColor',borderRadius:'50%',animation:'spin 0.7s linear infinite',display:'inline-block'}}/> : '↓ Пополнить'}
              </button>
            </div>
          </div>
        </div>
