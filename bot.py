...
AI_MAP = {"claude": ask_claude, "gemini": ask_gemini, "deepseek": ask_deepseek}
...
async def cmd_ai(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ai = ctx.args[0].lower()
    if ai not in AI_MAP:
        await update.message.reply_text("некорректно. claude, gemini или deepseek")
        return
    config["active_ai"] = ai
    save_config(config)
    desc = {
...
def main():
    ...
    # Запуск: webhook на Railway, polling локально
    if WEBHOOK_URL:
        log.info(f"🚀 Запуск через webhook: {WEBHOOK_URL}")
        app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
        ...
        log.info("🚀 Запуск через polling (локально)")
